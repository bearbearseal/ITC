...
namespace TestReadWrite
{
	...
	void write_message_thread(shared_ptr<ITC<string>::FixedSocket>* socket) {
		...
		for (unsigned i = 0; i < 2000; ++i) {
			...
			while ((*socket)->has_message()) {
				...
				++replyCount;
				printf("Socket %u: Got \"%s\" from socket %u, replyCount %u\n", 
	...
	void test_2_vs_1() {
		...
		auto socket1 = itc->create_socket(1);
		auto socket2 = itc->create_fixed_socket(2, 1);
		auto socket3 = itc->create_fixed_socket(3, 1);
		auto socket4 = itc->create_fixed_socket(4, 1);
